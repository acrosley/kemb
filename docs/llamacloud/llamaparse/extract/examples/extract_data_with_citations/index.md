---
title: Extract Data from Financial Reports - with Citations and Reasoning | Developer Documentation
---
...
In this example, we’ll be using [LlamaExtract](https://developers.llamaindex.ai/llamaparse/extract/getting_started?utm_campaign=extract\&utm_medium=recipe) to extract structured data from an SEC filing (specifically, the filing by Nvidia for fiscal year 2025).

On top of simple data extraction, we’ll enable citations and reasoning for each extracted field. This allows us to:
...
## Extract Data with LlamaExtract
...
- [python](#tab-panel-144)
- [typescript](#tab-panel-145)
...
We’ll enable `cite_sources` in the extraction config so we can trace every value back to its origin in the document and understand how the LLM arrived at each extracted value.

- [python](#tab-panel-146)
- [typescript](#tab-panel-147)
...
- [python](#tab-panel-148)
- [typescript](#tab-panel-149)
...
```
// Fetch metadata (not returned by default)
const detailed = await client.extract.get(job.id, { expand: ["extract_metadata"] });
console.log(detailed.extract_metadata);
```
...
In this example, we used LlamaExtract to extract data with citations and reasoning from a financial document. To further customize and improve on the results, you can also try adding a `system_prompt` to the extraction config.

#### Learn More

- [LlamaExtract Documentation](https://developers.llamaindex.ai/llamaparse/extract/getting_started)
- [Example Notebooks](https://github.com/run-llama/llama-cloud-py/tree/main/examples/extract)
