...
```
pip install llama-cloud>=2.1 requests llama-index-workflows
```
...
print(f"Split job completed with status: {response.status}")
print()
...
The extraction configuration specifies:

- **`tier`**: `agentic` for highest quality extraction
- **`target_pages`**: Extract from specific pages (e.g., “5” for the resume on page 5)
- **`confidence_scores`**: Include confidence scores in results

We then create an extraction job and poll for completion.
...
extracted_result = ResumeSchema.model_validate(job.extract_result)
```
    ...
    async def extract_resume(self, ev: ExtractResume, ctx: Context) -> StopEvent:
        ...
        for event in ready:
            if event.category == "resume":
                ...
                job = await self.client.extract.create(
                    file_input=file_id,
                    configuration={
                        "data_schema": self.extract_schema.model_json_schema(),
                        "tier": "agentic",
                        "target_pages": f"{min(event.pages)}-{max(event.pages)}",
                    },
                )
                while job.status not in ("COMPLETED", "FAILED", "CANCELLED"):
                    await asyncio.sleep(2)
                    job = await self.client.extract.get(job.id)
                extraction_result.append(job.extract_result)
